...
import { fetchGoogleAddress } from './google-maps'

// Map locale string → Faker instance
const localeMap: Record<string, Faker> = {
  ...
  en_ZA: fakerEN_ZA,
  en_GH: fakerEN_GH,
  en_KE: fakerEN, // no specific KE locale, use generic EN
  en_NG: fakerEN_NG,
  ja: fakerJA,
  zh_CN: fakerZH_CN,
  zh_TW: fakerZH_TW,
...
export async function generateFakeProfile(countryCode: string): Promise<FakeProfile> {
  const config = countryConfigs[countryCode]
  if (!config) {
  ...
  // Try Google Maps first for authentic address data, fallback to Faker
  let street: string, city: string, state: string, zip: string

  const googleAddr = await fetchGoogleAddress(countryCode)
  if (googleAddr && googleAddr.city) {
    street = googleAddr.street
    city = googleAddr.city
    state = googleAddr.state
    zip = googleAddr.zip
  } else {
